...
export const FactureDetailView: React.FC<FactureDetailViewProps> = ({ invoice, suppliers, buildings, updateInvoice, onBack }) => {
    ...
    const renderEditField = (label: string, name: keyof Invoice, type: string, options?: any[]) => {
        ...
        return (
             <div>
                <label className="block text-sm font-medium text-gray-700">{label}</label>
                {/* FIX: Removed type cast to allow for better type compatibility with input element props. */}
                <input type={type} name={name} value={editedInvoice[name]} onChange={handleChange} className={commonProps} step={type === 'number' ? '0.01' : undefined} />
            </div>
        )
    };
